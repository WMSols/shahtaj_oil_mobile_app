...
class RmRecordCollectionController extends GetxController with CachedLoadMixin {
  RmRecordCollectionController(this._store);
  ...
  void _seedAmountControllers(List<RmInvoiceModel> rows) {
    for (final controller in invoiceAmountControllers.values) {
      controller.dispose();
    }
  ...
  void fillInvoiceRemaining(RmInvoiceModel invoice) {
  ...
  Future<void> submit() async {
    ...
    final confirmed = await AppConfirmSheet.show(
      title: AppTexts.rmConfirmCollection,
      message: AppTexts.rmConfirmCollectionMessage(
        shop.value?.name ?? '',
        _formatAmount(collectingTotal),
        method.value.label,
      ),
      confirmLabel: AppTexts.rmConfirmCollection,
    );
    ...
    try {
      ...
      AppToast.showSuccess(
        AppTexts.rmCollectionRecorded(recorded.receiptNumber),
      );
      Get.back(result: true);
    } catch (_) {
      AppToast.showError(AppTexts.rmCollectionFailed);
    } finally {
  ...
  String? _validate() {
    if (invoices.isEmpty) return AppTexts.rmSelectInvoicesHint;
    if (isBatch) {
      final amount = _parseAmount(batchAmountController.text);
      if (amount <= 0) return AppTexts.rmAmountRequired;
      if (amount > remainingTotal) return AppTexts.rmAmountExceedsRemaining;
    } else {
      var total = 0.0;
      for (final invoice in invoices) {
        ...
        if (amount > invoice.remainingAmount) {
          return AppTexts.rmAmountExceedsRemaining;
        }
        total += amount;
      }
      if (total <= 0) return AppTexts.rmAmountRequired;
    }

    final reference = referenceController.text.trim();
    if (method.value == PaymentMethod.cheque && reference.isEmpty) {
      return AppTexts.rmChequeNumberRequired;
    }
    if (method.value == PaymentMethod.bank && reference.isEmpty) {
      return AppTexts.rmBankReferenceRequired;
    }
    return null;
  }
